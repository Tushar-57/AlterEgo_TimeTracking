...
### Deep Agent Architecture Integration

#### Core Deep Agent Principles

The system integrates the four foundational Deep Agent principles:

1. **System Prompts**: Each agent uses carefully crafted system prompts based on Claude Code patterns, providing detailed instructions for tool usage, planning, and sub-agent coordination.

2. **Planning Tools**: Built-in TODO management system that allows agents to:
   - Break down complex tasks into manageable steps
   - Track progress through multi-step workflows
   - Maintain focus on current objectives
   - Update task status as work progresses

3. **Virtual File System**: Context offloading mechanism that:
   - Stores detailed information in virtual files using LangGraph state
   - Returns only summaries to agents to prevent context overflow
   - Enables persistent information across conversation turns
   - Supports file operations (read, write, edit, list) without actual filesystem access

4. **Sub-Agent Delegation**: Context isolation through specialized sub-agents:
   - Each sub-agent operates with clean, isolated context
   - Task delegation prevents context pollution in parent agents
   - Results are returned as tool messages to maintain separation
   - Enables parallel processing of independent tasks

#### Deep Agent State Schema

```python
class DeepAgentState(AgentState):
    """Extended state for Deep Agent capabilities."""
    # Core agent state
    ...
    interrupt_config: Dict[str, HumanInterruptConfig] = {}

class Todo(TypedDict):
    """Task item for planning and progress tracking."""
    content: str
...
Each agent type gets enhanced with Deep Agent tools:

**Built-in Tools (All Agents):**
- `write_todos`: Plan and track task progress
- `read_todos`: Review current task list
- `write_file`: Store information in virtual filesystem
...
- `delegate_task`: Delegate work to specialized sub-agents
- `coordinate_agents`: Manage multi-agent workflows
- `synthesize_results`: Combine outputs from multiple agents

**Domain-Specific Tools per Agent:**
- **Productivity**: Task management, goal tracking, time analysis
- **Health**: Habit tracking, wellness monitoring, routine management
- **Finance**: Expense tracking, budget analysis, financial planning
- **Scheduling**: Calendar integration, appointment booking, time optimization
- **Journal**: Reflection prompts, mood tracking, growth insights

#### Sub-Agent Architecture

```python
# Sub-agent registry for context isolation
SPECIALIZED_SUBAGENTS = {
    "research-agent": {
        "name": "research-agent",
        "description": "Conduct focused research on specific topics",
        "prompt": RESEARCH_AGENT_PROMPT,
        "tools": ["web_search", "think_tool", "write_file", "read_file"],
        "capabilities": ["information_gathering", "source_analysis"]
    },
    "analysis-agent": {
        "name": "analysis-agent", 
        "description": "Analyze data and provide insights",
        "prompt": ANALYSIS_AGENT_PROMPT,
        "tools": ["read_file", "write_file", "think_tool", "data_analysis"],
    ...
    "synthesis-agent": {
        ...
        "capabilities": ["information_synthesis", "report_generation"]
    }
}
```
...
class HumanInterruptConfig(TypedDict):
    """Configuration for human approval workflows."""
    allow_ignore: bool = False  # Skip tool execution
    allow_respond: bool = True   # Provide alternative response
    allow_edit: bool = True      # Modify tool arguments
    allow_accept: bool = True    # Approve as-is

# Tools requiring human approval
APPROVAL_REQUIRED_TOOLS = {
    "google_sheets_write": True,
    "google_calendar_create": True,
    "send_notification": {
        "allow_ignore": False,
        "allow_respond": True,
        "allow_edit": True,
