...
public class TimerController {
	...
	@PostMapping("/start")
	public ResponseEntity<?> startTimeEntry(@Valid @RequestBody StartTimeEntryRequest request,
		...
		try {
			...
			return ResponseEntity.internalServerError()
					.body(Map.of("error", "Creation failed", "message", e.getMessage()));
		}
	}
...
//	}
	...
	@GetMapping("/active")
	public ResponseEntity<?> getActiveTimer(Authentication auth) {
		try {
	...
	@PostMapping("/{id}/stop")
	public ResponseEntity<TimeEntry> stopTimer(@PathVariable Long id,
			@RequestParam(required = false) LocalDateTime manualEnd, Authentication auth) {
		Users user = userDetailsService.getCurrentUser(auth);
